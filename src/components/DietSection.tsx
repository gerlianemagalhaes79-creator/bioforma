...
import { calculateAllVitaminsForFood } from '../data/nutritionData';
...
export default function DietSection({ user, profile, onNavigateTab }: DietSectionProps) {
  const [diets, setDiets] = useState<any[]>([]);
  const [supplements, setSupplements] = useState<any[]>([]);
  const [showAddSupplement, setShowAddSupplement] = useState(false);
  const [activeVitaminTip, setActiveVitaminTip] = useState<string | null>(null);
  const [showVitaminInspector, setShowVitaminInspector] = useState(false);
  const [viewingMealVitamins, setViewingMealVitamins] = useState<any | null>(null);
  ...
  const handleSearchNutrition = async (index: number) => {
    ...
    try {
      ...
      if (resp && resp.ok) {
        ...
        if (resData.success && resData.data) {
          ...
          // Calculate/augment with all vitamins helper if needed
          const calcVits = calculateAllVitaminsForFood(meal.name, gWeight);
          const completeVitamins = nutData.allVitamins || nutData.vitamins || calcVits.allVitamins;
          ...
          updatedMeals[index] = {
            ...
            vitaminA: parseFloat((nutData.vitaminA ?? calcVits.vitaminA).toFixed(1)),
            vitaminB1: parseFloat((nutData.vitaminB1 ?? calcVits.vitaminB1).toFixed(2)),
            vitaminB2: parseFloat((nutData.vitaminB2 ?? calcVits.vitaminB2).toFixed(2)),
            vitaminB3: parseFloat((nutData.vitaminB3 ?? calcVits.vitaminB3).toFixed(2)),
            vitaminB5: parseFloat((nutData.vitaminB5 ?? calcVits.vitaminB5).toFixed(2)),
            vitaminB6: parseFloat((nutData.vitaminB6 ?? calcVits.vitaminB6).toFixed(2)),
            vitaminB7: parseFloat((nutData.vitaminB7 ?? calcVits.vitaminB7).toFixed(1)),
            vitaminB9: parseFloat((nutData.vitaminB9 ?? calcVits.vitaminB9).toFixed(1)),
            vitaminB12: parseFloat((nutData.vitaminB12 ?? calcVits.vitaminB12).toFixed(2)),
            vitaminC: parseFloat((nutData.vitaminC ?? calcVits.vitaminC).toFixed(1)),
            vitaminD: parseFloat((nutData.vitaminD ?? calcVits.vitaminD).toFixed(1)),
            vitaminE: parseFloat((nutData.vitaminE ?? calcVits.vitaminE).toFixed(2)),
            vitaminK: parseFloat((nutData.vitaminK ?? calcVits.vitaminK).toFixed(1)),
            choline: parseFloat((nutData.choline ?? calcVits.choline).toFixed(1)),
            allVitamins: completeVitamins,
            absorptionTip: nutData.absorptionTip || calcVits.absorptionTip,
    ...
    if (!success) {
      try {
        const nutData = calculateAllVitaminsForFood(meal.name, gWeight);
        const updatedMeals = [...newDiet.meals];
        updatedMeals[index] = {
          ...updatedMeals[index],
          calories: Math.round(nutData.calories || 0),
          ...
          vitaminB1: parseFloat((nutData.vitaminB1 || 0).toFixed(2)),
          vitaminB2: parseFloat((nutData.vitaminB2 || 0).toFixed(2)),
          vitaminB3: parseFloat((nutData.vitaminB3 || 0).toFixed(2)),
          vitaminB5: parseFloat((nutData.vitaminB5 || 0).toFixed(2)),
          vitaminB6: parseFloat((nutData.vitaminB6 || 0).toFixed(2)),
          vitaminB7: parseFloat((nutData.vitaminB7 || 0).toFixed(1)),
          vitaminB9: parseFloat((nutData.vitaminB9 || 0).toFixed(1)),
          vitaminB12: parseFloat((nutData.vitaminB12 || 0).toFixed(2)),
          vitaminC: parseFloat((nutData.vitaminC || 0).toFixed(1)),
          vitaminD: parseFloat((nutData.vitaminD || 0).toFixed(1)),
          vitaminE: parseFloat((nutData.vitaminE || 0).toFixed(2)),
          vitaminK: parseFloat((nutData.vitaminK || 0).toFixed(1)),
          choline: parseFloat((nutData.choline || 0).toFixed(1)),
          allVitamins: nutData.allVitamins,
          absorptionTip: nutData.absorptionTip,
          source: `${nutData.source || 'Tabela Oficial Local'}`
        };
        setNewDiet({
          ...newDiet,
          meals: updatedMeals
  ...
  return (
    <div className="space-y-6 animate-fade-in text-zinc-800">
      ...
      <div className="flex flex-col gap-4">
        ...
        {/* NUTRITIONIST & VITAMIN INSPECTOR SHORTCUT CARDS */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {/* Vitamin Inspector Trigger */}
          <div className="bg-gradient-to-br from-amber-500/10 via-pink-500/5 to-purple-500/10 p-5 rounded-3xl border border-amber-200/70 shadow-xs flex flex-col justify-between space-y-3">
            <div className="flex items-start gap-3">
              <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-amber-500 to-pink-500 flex items-center justify-center text-white shrink-0 shadow-md shadow-amber-200">
                <Sparkles size={18} />
              </div>
              <div>
                <span className="text-[9px] font-black uppercase tracking-widest text-amber-700 block">
                  Bioquímica & Micronutrientes
                </span>
                <h4 className="text-sm font-black text-zinc-900">
                  Puxador de Vitaminas
                </h4>
                <p className="text-xs text-zinc-500 leading-snug mt-0.5">
                  Descubra instantaneamente todas as vitaminas presentes em qualquer alimento ou receita.
                </p>
              </div>
            </div>
            <button
              type="button"
              onClick={() => setShowVitaminInspector(true)}
              className="w-full py-2.5 bg-gradient-to-r from-amber-500 to-pink-500 text-white font-extrabold uppercase text-xs rounded-xl shadow-sm hover:opacity-95 transition-all cursor-pointer flex items-center justify-center gap-1.5 border-0"
            >
              <Sparkles size={14} />
              <span>Puxar Vitaminas de um Alimento</span>
            </button>
          </div>

          {/* Nutritionist Persona Consultation Trigger */}
          <div className="bg-gradient-to-br from-zinc-900 to-zinc-950 text-white p-5 rounded-3xl border border-zinc-800 shadow-md flex flex-col justify-between space-y-3">
            <div className="flex items-start gap-3">
              <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-pink-500 to-rose-500 flex items-center justify-center text-white shrink-0 shadow-md shadow-pink-500/30">
                <Apple size={18} />
              </div>
              <div>
                <div className="flex items-center gap-1.5">
                  <span className="text-[9px] font-black uppercase tracking-widest text-pink-300">
                    Nutricionista Clínico IA
                  </span>
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
                </div>
                <h4 className="text-sm font-black text-zinc-100">
                  Consultório & Especialidades
                </h4>
                <p className="text-xs text-zinc-400 leading-snug mt-0.5">
                  Hipertrofia, SOP, emagrecimento e ganho muscular incorporados.
                </p>
              </div>
            </div>
            <button
              type="button"
              onClick={() => onNavigateTab ? onNavigateTab('nutritionist') : null}
              className="w-full py-2.5 bg-zinc-800 hover:bg-zinc-700 text-pink-300 font-extrabold uppercase text-xs rounded-xl border border-pink-500/30 hover:border-pink-500/60 transition-all cursor-pointer flex items-center justify-center gap-1.5"
            >
              <Apple size={14} />
              <span>Falar com o Nutricionista</span>
            </button>
          </div>
        </div>
      </div>
                              ...
                              <div className="space-y-2">
                                {diet.meals.map((meal: any, i: number) => (
                                  <div key={i} className="bg-pink-50/20 rounded-xl p-3 border border-pink-50/40 space-y-1 animate-fade-in">
                                    ...
                                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[9px] text-zinc-400 font-bold">
                                      ...
                                      <button
                                        type="button"
                                        onClick={() => setViewingMealVitamins(meal)}
                                        className="text-[8.5px] bg-amber-50 hover:bg-amber-100 text-amber-800 px-2 py-0.5 rounded-md font-extrabold flex items-center gap-1 border border-amber-200/80 cursor-pointer transition-all ml-auto"
                                      >
                                        <Sparkles size={10} className="text-amber-600" />
                                        <span>Ver Vitaminas</span>
                                      </button>
                                    </div>
                                  </div>
                                ))}
      ...
      {/* MODAL: FOOD VITAMIN INSPECTOR */}
      <AnimatePresence>
        {showVitaminInspector && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            <motion.div
              ...
              className="bg-white rounded-3xl max-w-lg w-full max-h-[90vh] overflow-y-auto border border-pink-100 shadow-2xl p-5 space-y-4 my-8"
            >
              <div className="flex justify-between items-center border-b border-pink-50 pb-3">
                <div className="flex items-center gap-2 text-pink-600">
                  <Sparkles size={20} />
                  <h3 className="font-extrabold text-zinc-900 text-lg">Puxador Completo de Vitaminas</h3>
                </div>
                <button
                  type="button"
                  onClick={() => setShowVitaminInspector(false)}
              ...
              <FoodVitaminInspector
                onAddFoodToDiet={(foodData) => {
                  const mealToAdd: DietMealItem = {
                    name: foodData.name,
                    weight: foodData.weight || 100,
                    unit: 'g',
                    calories: foodData.calories || foodData.kcal || 0,
                    protein: foodData.protein || foodData.p || 0,
                    carbs: foodData.carbs || foodData.c || 0,
                    fat: foodData.fat || foodData.f || 0,
                    ...
                    vitaminB1: foodData.vitaminB1 || 0,
                    vitaminB2: foodData.vitaminB2 || 0,
                    vitaminB3: foodData.vitaminB3 || 0,
                    vitaminB5: foodData.vitaminB5 || 0,
                    vitaminB6: foodData.vitaminB6 || 0,
                    vitaminB7: foodData.vitaminB7 || 0,
                    vitaminB9: foodData.vitaminB9 || 0,
                    vitaminB12: foodData.vitaminB12 || 0,
                    vitaminC: foodData.vitaminC || 0,
                    vitaminD: foodData.vitaminD || 0,
                    vitaminE: foodData.vitaminE || 0,
                    vitaminK: foodData.vitaminK || 0,
                    choline: foodData.choline || 0,
                    allVitamins: foodData.allVitamins,
                    absorptionTip: foodData.absorptionTip,
                    source: foodData.source || 'Puxador de Vitaminas'
                  };
                  setNewDiet(prev => ({
                    ...prev,
                    meals: [...prev.meals.filter(m => m.name.trim() !== ''), mealToAdd]
                  }));
                  setShowVitaminInspector(false);
                  setShowAddModal(true);
                }}
              />
            </motion.div>
      ...
      {/* MODAL: VIEW SPECIFIC MEAL VITAMINS BREAKDOWN */}
      <AnimatePresence>
        {viewingMealVitamins && (() => {
          const calcData = calculateAllVitaminsForFood(
            viewingMealVitamins.name, 
            viewingMealVitamins.weight || 100
          );
          const vitsList = viewingMealVitamins.allVitamins && viewingMealVitamins.allVitamins.length > 0 
            ? viewingMealVitamins.allVitamins 
            : calcData.allVitamins;

          const lipo = vitsList.filter((v: any) => v.solubility === 'Lipossolúvel' || v.solubility === 'lipossolúvel');
          const hidro = vitsList.filter((v: any) => v.solubility === 'Hidrossolúvel' || v.solubility === 'hidrossolúvel');
          ...
          return (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="fixed inset-0 z-[105] bg-black/70 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto"
            >
              <motion.div
                initial={{ scale: 0.95, y: 20 }}
                animate={{ scale: 1, y: 0 }}
                exit={{ scale: 0.95, y: 20 }}
                className="bg-white rounded-3xl max-w-md w-full max-h-[90vh] overflow-y-auto border border-pink-100 shadow-2xl p-5 space-y-4 my-8"
              >
                <div className="flex justify-between items-start border-b border-pink-50 pb-3">
                  <div>
                    <div className="flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest text-pink-500">
                      <Sparkles size={12} />
                      Perfil Vitamínico Completo
                    </div>
                    <h3 className="font-extrabold text-zinc-900 text-lg">
                      {viewingMealVitamins.name}
                    </h3>
                    <p className="text-xs text-zinc-400 font-bold">
                      Porção avaliada: {viewingMealVitamins.weight || 100}{viewingMealVitamins.unit || 'g'} • {viewingMealVitamins.calories || calcData.calories || calcData.kcal} kcal
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={() => setViewingMealVitamins(null)}
                    className="p-1.5 text-zinc-400 hover:text-zinc-700 rounded-full hover:bg-zinc-100 cursor-pointer"
                  >
                    <X size={20} />
                  </button>
                ...
                {/* Lipossolúveis */}
                <div className="space-y-2">
                  <span className="text-[10px] font-black uppercase tracking-wider text-amber-700 flex items-center gap-1.5 bg-amber-50 px-2.5 py-1 rounded-lg w-fit">
                    Vitaminas Lipossolúveis (A, D, E, K)
                  </span>
                  <div className="grid grid-cols-2 gap-2">
                    {lipo.map((v: any, idx: number) => (
                      <div key={idx} className="bg-amber-50/40 p-2.5 rounded-xl border border-amber-100/70">
                        <div className="flex justify-between items-center">
                          <span className="text-xs font-black text-zinc-800">{v.name}</span>
                          <span className="text-[10px] font-extrabold text-amber-600 font-mono">{v.dailyValuePct}% VD</span>
                        </div>
                        <p className="text-[11px] font-bold text-zinc-600 mt-0.5">{v.amount} {v.unit}</p>
                        <p className="text-[9px] text-zinc-400 leading-tight mt-1">{v.function}</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {/* Hidrossolúveis */}
                <div className="space-y-2">
                  <span className="text-[10px] font-black uppercase tracking-wider text-sky-700 flex items-center gap-1.5 bg-sky-50 px-2.5 py-1 rounded-lg w-fit">
                    Vitaminas Hidrossolúveis (Complexo B & C)
                  </span>
                  <div className="grid grid-cols-2 gap-2">
                    {hidro.map((v: any, idx: number) => (
                      <div key={idx} className="bg-sky-50/40 p-2.5 rounded-xl border border-sky-100/70">
                        <div className="flex justify-between items-center">
                          <span className="text-xs font-black text-zinc-800">{v.name}</span>
                          <span className="text-[10px] font-extrabold text-sky-600 font-mono">{v.dailyValuePct}% VD</span>
                        </div>
                        <p className="text-[11px] font-bold text-zinc-600 mt-0.5">{v.amount} {v.unit}</p>
                        <p className="text-[9px] text-zinc-400 leading-tight mt-1">{v.function}</p>
                      </div>
                ...
                {/* Dica de Absorção */}
                {(viewingMealVitamins.absorptionTip || calcData.absorptionTip) && (
                  <div className="bg-gradient-to-r from-pink-50 to-rose-50 border border-pink-200/60 p-3 rounded-xl text-xs text-pink-900 leading-relaxed">
                    <span className="font-extrabold block text-[10px] uppercase tracking-wider text-pink-700 mb-1">
                      💡 Dica de Biodisponibilidade
                    </span>
                    {viewingMealVitamins.absorptionTip || calcData.absorptionTip}
                  </div>
                )}
                ...
                <button
                  type="button"
                  onClick={() => setViewingMealVitamins(null)}
                  className="w-full py-3 bg-zinc-900 text-white rounded-xl font-bold uppercase text-xs hover:bg-zinc-800 transition-all cursor-pointer"
                >
                  Fechar Detalhes
                </button>
              </motion.div>
